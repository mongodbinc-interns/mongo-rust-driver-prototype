...
#[cfg(feature = "ssl")]
use std::any::Any;
// Necessary to use openssl::ssl::HandshakeError::description(), apparently
#[cfg(feature = "ssl")]
...
impl From<ErrorStack> for Error {
    fn from(error_stack: ErrorStack) -> Error {
        let mut error_message = String::from("An error occurred during an SSL operation");

        if let Some(ref err) = error_stack.errors().last() {
            if let Some(function) = err.function() {
                error_message.push_str(" in the function `");
                error_message.push_str(function);
                error_message.push('`');
            }
        ...
        Error::SslError(error_message)
    }
}

#[cfg(feature = "ssl")]
impl<T: Any + fmt::Debug> From<HandshakeError<T>> for Error {
    fn from(err: HandshakeError<T>) -> Error {
        Error::SslError(String::from(err.description()))
    }
}
...
impl fmt::Display for Error {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            #[cfg(feature = "ssl")]
            Error::SslError(ref err) => write!(fmt, "{}", err),
        }
    }
}
impl error::Error for Error {
    fn description(&self) -> &str {
        match *self {
            ...
            #[cfg(feature = "ssl")]
            Error::SslError(ref inner) => inner,
        }
    }
